...
! AUTHORS:
     ...
     SUBROUTINE hdsvm_cd (alpha, lam2, hval, nobs, nvars, x, y, jd, & 
      & pfncol, pf, pf2, dfmax, pmax, nlam, flmin, ulam, eps, isd, maxit, &
      & nalam, b0, beta, ibeta, nbeta, alam, npass, jerr, sigma, is_exact) 
...
!------- first standardize the data -----------------------------
         ...
         CALL hdsvm_cd_path (alpha, lam2, hval, maj, mval, nobs, nvars, &
...
!---------- lambda loop -----------------------------------------
         ...
         !find the intercept
         CALL objfun_enetsvm(b(0), bb, ab, ka, y, 0.0D0, 0.0D0, nobs, obj0)
         CALL opt_int_enetsvm(-1.0D2, 1.0D2, nobs, ab, ka, bb, y, 0.0D0, 0.0D0, &
                  & obj1, b00)
         intercept = b00

         al0 = 0.0D0
         vl = 0.0D0
         CALL svm_drv (nobs, nvars, x, y, y * intercept, vl, 1/1.0D-9, 1 - 1.0D-9, 1 + 1.0D-9)
         ga = Abs (vl)
   ...
   ! \sum_{i=1}^n V'(r_i)y_i x_{ij}
                        DO i = 1, nobs   
                           IF (r(i) .LE. onemh) THEN
                              dl(i) = -1.0D0
                           ELSEIF (r(i) < oneph) THEN
                              dl(i) = 0.5D0 * hinv * (r(i) - oneph)
                           ELSE
                              dl(i) = 0.0D0
                           ENDIF
                           u = u + dl(i) * y(i) * x(i, k)
   ...
   !---------- 2. update intercept ( in inner loop) ----------------
               ...
               ka = y * r - b(0)!Matmul(x, b(1:nvars))
               bb = Dot_product(b(1:nvars), b(1:nvars))
               CALL objfun_enetsvm(b(0), bb, ab, ka, y, al, lam2, nobs, obj0)
               CALL opt_int_enetsvm(-1.0D2, 1.0D2, nobs, ab, ka, bb, y, al, lam2, &
                  & obj1, b00)
      ...
      ! M \tilde{\beta_j} - 1/n * (\sum_{i=1}^n V'(r_i)y_i x_{ij})
                                 ...
                                 IF (si>0) THEN
                                    ...
                                    mb = sigma * sx(k) * b(k) + DOT_PRODUCT(x(sset(1:si),k), &
                                      & (theta(1:si) + sigma * (1.0D0-r(sset(1:si)))) &
                                      & /y(sset(1:si)) )               
                                 ENDIF
      ...
      !---------- 2. update intercept ( in middle loop) ---------------
                        IF (ni > pmax) EXIT
                           ...
                           IF (si > 0)  THEN 
                              d = d + SUM((theta(1:si) + &
                                 & sigma * (1.0D0 - r(sset(1:si)))) / y(sset(1:si))) 
                              d = d / (sigma * si + mval) 
                           ELSE 
                              d = d/mval
                           ENDIF
      ...
      !---------- 3. update theta ( in middle loop) ---------------
                        IF (si > 0) theta(1:si) = theta(1:si) + sigma * (1.0D0 - r(sset(1:si)))
                        IF (dif < eps) EXIT
                        IF (SUM(npass) > maxit) THEN
                           jerr = -1
                           RETURN
